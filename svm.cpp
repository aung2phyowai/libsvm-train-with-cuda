...
public:
	...
	:Kernel(prob.l, prob.x, param)
	{
		...
		CUDA_init_SVC_Q(prob.l, x, y, x_square, param);
	}
	...
	Qfloat *get_Q(int i, int len) const
	{
		...
		{
			CUDA_get_Q(i, start, len, data + start);
/*
			// Verify result
			for(int j=start;j<len;j++)
			{
				float val = (Qfloat)(y[i]*y[j]*(this->*kernel_function)(i,j));
				if( val != data[j] )
					printf("i:%d, j:%d, val:%f, cudaval:%f\n", i, j, val, data[j]);
			}
*/
/*
		// original implement
...
*/
	...
	{
		printf("Please run with -h 0(disable shrinking heuristics)\n");
		exit(0);		
